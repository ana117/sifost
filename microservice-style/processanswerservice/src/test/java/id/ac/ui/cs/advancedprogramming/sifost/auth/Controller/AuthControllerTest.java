...
class AuthControllerTest {
    ...
    void testGetCreateAdminPage() throws Exception {
        mockMvc.perform(get("/create-admin"))
                .andExpect(status().isOk());
    }
    ...
    void testCreateAdminSuccessfully() throws Exception {
        ...
        when(authService.existDuplicate(null)).thenReturn(false);

        mockMvc.perform(post("/create-admin")
                        .sessionAttr(TOKEN_ATTR_NAME,csrfToken)
                        .param(csrfToken.getParameterName(),csrfToken.getToken()))
                .andExpect((handler().methodName("createAdmin")));

        verify(authService, times(1)).registerAdmin(any(UserDTO.class));
    }

    @Test
    @WithMockUser(authorities = "ADMIN")
    void testCreateAdminWithDuplicateName() throws Exception {
        var TOKEN_ATTR_NAME = "org.springframework.security.web.csrf.HttpSessionCsrfTokenRepository.CSRF_TOKEN";
        var httpSessionCsrfTokenRepository = new HttpSessionCsrfTokenRepository();
        var csrfToken = httpSessionCsrfTokenRepository.generateToken(new MockHttpServletRequest());

        when(authService.existDuplicate(null)).thenReturn(true);
        ...
        verify(authService, times(0)).registerAdmin(any(UserDTO.class));
    }
}
