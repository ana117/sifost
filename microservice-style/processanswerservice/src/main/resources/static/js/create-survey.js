...
async function submitForm(e) {
    ...
    const pickedInterest = displayRadioValue();

    const questionsContainer = document.getElementById('questions');
    const questions = questionsContainer.childNodes;

    const jsonMap = {'title': surveyTitle,'interest': pickedInterest, 'questions': [], 'ownerId': ownerId};
...
function displayRadioValue() {
    let pickedInterest = document.getElementsByName('inlineRadioOptions');
    let res = pickedInterest.length;
    for (i = 0; i<pickedInterest.length;i++){
        if (pickedInterest[i].checked){
            res = pickedInterest[i].value;
        }
    }
    return res;
}
...
function createMultipleChoiceQuestion(questionBody, id) {
    ...
    addAnswerButton.classList.add('btn', 'btn-dark');
